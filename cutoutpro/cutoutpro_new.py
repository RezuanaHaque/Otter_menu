...
class Automation:
    ...
    def run(self):
        ...
        self.cj = browser_cookie3.chrome(domain_name='picup.shop')
    ...
    def _prepareBrowser(self, headless=True):
        ...
        self.driver.get("https://www.picup.shop/photo-editing-background.html")

        self.driver.maximize_window()

        for cookie in self.cj:  # session cookies
            if cookie.domain.startswith('.') or cookie.domain.startswith('www.'):
        ...
        self.driver.execute_script("window.localStorage.setItem(arguments[0], arguments[1]);", 'user_Token', token)
        
        self.driver.get("https://www.picup.shop/photo-editing-background.html")
        
        try:
            profileIcon = WebDriverWait(self.driver, 10).until(EC.element_to_be_clickable((By.XPATH, '//header/div/div[1]/div[1]/div/div/div/span')))
        except:
            print('login failed, please make sure you have logged in to https://www.picup.shop/photo-editing-background.html in Chrome before running this script')
            quit()
        
        if headless:
            print('logged in as {}'.format(profileIcon.text))
    ...
    def _generateImages(self, row):
        try:
            ...
            if len(self.hints) > 0:         
                ...
                uploadBtn = self.driver.find_element(By.XPATH, '//div[@class="oA waves-effect waves-light flex"]')
                click(self.driver, uploadBtn)
                imageInput.send_keys(filePath)

                # resize
                WebDriverWait(self.driver, 10).until(EC.element_to_be_clickable((By.XPATH, '//div[@class="resize"]')))
                resizeBtn = self.driver.find_element(By.XPATH, '//div[@class="resize"]')
                click(self.driver, resizeBtn)

                WebDriverWait(self.driver, 10).until(EC.element_to_be_clickable(('xpath', '//div[@class="el-input"]/input')))
                sizeInputs = self.driver.find_elements(By.XPATH, '//div[@class="el-input"]/input')
                size1 = sizeInputs[0]
                size2 = sizeInputs[1]
