...
class Automation:
    ...
    def run(self):
        ...
        while True:
            #             # x = input()
            x = 'https://docs.google.com/spreadsheets/d/1-SsCfiaq-U1TSXmPcgjoIPDDOzNJQlMccwRp65lA6sY/edit#gid=623556071'
            if len(x.split('/')) < 2:
        ...
        while True:
            # x = input()
            x = 'https://manager.tryotter.com/menus/brand/fbe02206-7595-49c1-b9dd-9d7a4d28781f'
            if x:
                self.menuUrl = x
                break
        ...
        try:
            ...
            if name not in self.items:
                ...
                if self.published:
                    ...
                    # print('res', res.json())
                    storeEntityUuid = res.json()["data"]["brandMenuTemplateEntities"]["entities"][0]["storeConfigurations"][0]["id"]
                else:
    ...
    def getMenuSummary(self):
        ...
        for entity in entities:
            ...
            if entity["__typename"] == 'MenuTemplateItem':
                ...
                itemInfo["description"] = entity["description"][0]["value"]
    ...
    def createModifierGroups(self, addons):
        ...
        for modifierGroup in addons.split('\n\n\n'):
            ...
            modifierGroupNumConstraint = ""
            modifierGroupSingleItemNumConstraint = ""
            if len(modifierGroupMeta.split('\n')) > 1:
            ...
            if modifierGroupNumConstraint.strip() != '':
                modifierGroupNumConstraints = modifierGroupNumConstraint.split('-')
                if len(modifierGroupNumConstraints) >= 1:
                    if modifierGroupNumConstraints[0].strip().isdigit():
                        minimumNumberOfChoices = int(modifierGroupNumConstraints[0].strip())
                if len(modifierGroupNumConstraints) >= 2 and modifierGroupNumConstraints[1].strip() != 'inf':
                    if modifierGroupNumConstraints[1].strip().isdigit():
                        maximumNumberOfChoices = int(modifierGroupNumConstraints[1].strip())

            maxPerModifierSelectionQuantity = 'inf'
            if modifierGroupSingleItemNumConstraint.strip() != '' and modifierGroupSingleItemNumConstraint.strip() != 'inf':
                if modifierGroupSingleItemNumConstraint.strip().isdigit():
    ...
    def modifyPrice(self, priceObject, price):
        price = price.replace('$', '')  # Remove the dollar sign from the price string
        if not price.isdigit():
            # Skip price modification if the value is not a valid price
            return
